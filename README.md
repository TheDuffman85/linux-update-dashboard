...
- **Localized UI:** Arabic, English, German, French, Hindi, Japanese, Portuguese, Spanish, Russian, and Chinese translations with browser-language detection
...
| `LUDASH_MAX_SSH_TIMEOUT`               | No       | `120`                   | Maximum allowed SSH connection timeout in seconds enforced by the API                                                                                                             |
| `LUDASH_MAX_CMD_TIMEOUT`               | No       | `600`                   | Maximum allowed SSH command execution timeout in seconds enforced by the API                                                                                                      |
...
Linux Update Dashboard currently includes Arabic, English, German, French, Hindi, Japanese, Portuguese, Spanish, Russian, and Chinese UI translations. Browser-language detection applies to the browser UI. Server-generated text such as notification bodies and scheduled messages can only use a translated language when the language setting is explicitly set; with **Browser default**, those server-generated messages are sent in English. Translation help is very welcome: corrections, more natural wording, and new languages can be contributed by updating the JSON files in `client/locales/` and keeping the same keys as `client/locales/en.json`.
